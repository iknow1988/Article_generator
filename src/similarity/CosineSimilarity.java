...
public class CosineSimilarity {
	...
	public double Cosine_Similarity_Score(String Text1, String Text2) {
		...
		for (int i = 0; i < word_seq_text1.length; i++) {
			String tmp_wd = word_seq_text1[i].trim().toLowerCase();
			tmp_wd = tmp_wd.replaceAll("[^\\p{L}\\p{Nd}]+", "");
			if (tmp_wd.length() > 0) {
		...
		for (int i = 0; i < word_seq_text2.length; i++) {
			String tmp_wd = word_seq_text2[i].trim().toLowerCase();
			tmp_wd = tmp_wd.replaceAll("[^\\p{L}\\p{Nd}]+", "");
			if (tmp_wd.length() > 0) {
				if (word_freq_vector.containsKey(tmp_wd)) {
					values vals1 = word_freq_vector.get(tmp_wd);
	...
	public static boolean findSimilarity(ArrayList<String> collection,
			String newContent) {
		boolean result = false;
		double best = 0.0;
		for (String para : collection) {
			CosineSimilarity cs1 = new CosineSimilarity();
			double value = cs1.Cosine_Similarity_Score(para, newContent);
			if (value > best) {
				best = value;
			}
		}
	...
	public static Article removeDuplicateLines(Article article) {
		Article newArticle = new Article();
		ArrayList<String> collection = new ArrayList<String>();
		int i = 0;
		for (Paragraph para : article.getParagraphs()) {
			Paragraph newPara = new Paragraph(para.getParagraphId());
			for (Sentence sentence : para.getSentences()) {
				if (i == 0) {
					collection.add(sentence.getText());
					newPara.addSentence(sentence);
					i++;
					continue;
				}
				if (findSimilarity(collection, sentence.getText())) {
					newPara.addSentence(sentence);
					collection.add(sentence.getText());
				}
				i++;
			}
			if (newPara.getSentences().size() > 0) {
				newArticle.addParagraph(newPara);
			}
		}
		return newArticle;
	}
