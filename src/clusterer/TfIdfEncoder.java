...
public class TfIdfEncoder implements Encoder {
	...
	/**
	 * Construct a term frequency - inverse document frequency encoder. The
	 * encoder encodes documents into Vectors with the specified number of
	 * features.
	 */
	...
	/**
	 * Calculate word histogram for the provided document and store in the
	 * histogram field. To ensure a constant size histogram Vector the words are
	 * first hashed to an integer between 0 and numFeatures - 1.
	 */
	private void calcHistogram(Document document) {
		...
		for (Sentence sentence : document.getSentences()) {
			...
			if (words.length == 0) {
				histogram.increment(hashWord(sentence.getText()));
			} else {
	...
	/**
	 * Calculate inverse document frequency for the provided DocumentList. The
	 * inverse document frequency for a word i is defined as log(N/Ni) where N
	 * is the total number documents and Ni is the number of documents where
	 * word i occurs. This method requires that the document histogram for each
	...
	private void calcInverseDocumentFrequency(DocumentList documentList) {
		...
		inverseDocumentFrequency = documentFrequency.invert()
				.multiply(documentList.size()).log();
	}

	/**
	 * Encode the provided document using Term Frequency - Inverse Document
	 * Frequency. This method requires that the inverse document frequency and
	 * document word histograms have already been calculated.
	 */
	private void encode(Document document) {
		Vector tfidf = document.getHistogram()
				.divide(document.getHistogram().max())
	...
	/**
	 * Hash word into integer between 0 and numFeatures - 1. Used to form
